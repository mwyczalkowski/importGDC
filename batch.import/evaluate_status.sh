...
# batch file is generated by batch.import/batch_from_SR.sh
#   It is either given as a filename, or else if one of "WGS", "WXS", "RNA-Seq", the name is
#   assumed based on default data locations
...
while getopts ":uf:DO:L:C:" opt; do
  case $opt in
    u)  
      UUID_ONLY=1
      ;;
...
#   /gscmnt/gc2521/dinglab/mwyczalk/somatic-wrapper-data//GDC_import/import.config/CPTAC3.b1/WXS.batch.dat
    ...
    if [ ! -e $BATCH_TEST ]; then
        ...
        >&2 echo Using default batch file $BATCH_TEST 
        BATCH=$BATCH_TEST
    fi
fi
